...
import com.sg.common.codebuilder.builder.freemark.LowerFirstCharacter;
import com.sg.common.codebuilder.builder.freemark.UpperFirstCharacter;
...
public abstract class BaseCodeBuilder implements ICodeBuilder {
     ...
     * @param id   放入信息的标示
     */
    protected void setPath(Map<String, Object> map, String path, String id) {
        map.put(id, path);
    }

    /**
     * description:根据包路径获取包名(最后一个节点的名称)
     * @param packagePath  包路径
     */
    protected String getPackageName(String packagePath) {
        if (null != packagePath && !"".equals(packagePath)) {
            String[] nodes = packagePath.split(".");
    ...
    protected String getFilePath(String path, String name) {
    ...
    protected Configuration getConfiguration() {
        ...
        cfg.setClassLoaderForTemplateLoading(this.getClass().getClassLoader(), "codebuilderTemplate");
//        cfg.setClassForTemplateLoading(this.getClass(), getTemplateDir());
     ...
     * @return 目录路径
     */
    public abstract String getRootFilePath();

    public void buildCode() {
        buildCode(templateData);
    }

    public void buildCode(Map<String, Object> data) {
        String root = getRootFilePath() == null ? "" : File.separator + getRootFilePath();
        String path = root + data.get("url") ;
