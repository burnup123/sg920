...
public class CartRestController extends BaseController{
    ...
    */
    ...
    /**
    * 根据条件，获取多个
    * url: {ctx}/api/v1/cart/finds
    * type: get
    */
    @RequestMapping(value = "/find", method = RequestMethod.GET)
    public Object find() throws Exception {
        try {
            List list = cartService.find(this.getParamData(), null);
    ...
    */
    ...
    public Object query(@RequestParam(value = "size") int size,@RequestParam(value = "page") int page) throws Exception {
        try {
            ...
            Long count = cartService.count(terms);
            List list = cartService.page(pageRequest, terms, null);
            Map ret = new HashMap();
            ret.put("total",count);
            ret.put("rows",list);
    ...
    /**
    * 统计数量
    * url: {ctx}/api/v1/cart/count
    * type: get
    */
    @RequestMapping(value = "/count", method = RequestMethod.GET)
    public Object count(HttpServletRequest request) throws Exception {
        try {
            return ReturnUtil.getSucessInfo(cartService.count(this.getParamData()));
        } catch (Exception e) {
            logger.error(e.getMessage(), e);
            return ReturnUtil.getErrorInfo(e.getMessage());
        }
